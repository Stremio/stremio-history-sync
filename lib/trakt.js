...
const { MetaVideos,fetch } = require('./helper.js');

async function TraktData(access_token, endpoint = 'https://www.strem.io/trakt') {
    try {
        if (!access_token) throw "no access_token";
...
async function traktSync(authKey) {
    let API = new StremioAPIClient({ authKey })
    ...
    try {
        ...
        if (userData.trakt.expires_at < Date.now()/1000) throw "trakt token expired"
        ...
        if (datastorePut.success) {
            ...
            return {length: changes.length, movies: watchedMovies.length, series: WatchedSeries.length};
        } else {
...
function Watchedseries(element, meta) {
    if(!meta) return console.error(`error importing ${element.show.title}, couldn't retrive meta data`);
    let ids = meta.videos;
